...
module.exports.buildSVGSprite = function (spriteName, tasks, config) {
    ...
    var spriteData = {
        elements: [],
        path: config.spritePath + "/" + utils.joinName(config.prefix, spriteName, "sprite") + ".svg",
        sizes: {}
    };
    ...
    Object.keys(tasks).forEach(function (item) {
        resultsList.push({
            className: utils.joinName(config.prefix, item.slice(item.lastIndexOf("/") + 1, -suffix.length)),
            filename: item,
            svg: tasks[item]
        });
    });
