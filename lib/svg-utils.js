...
module.exports.buildSVGSprite = function (spriteName, tasks, config) {
    ...
    resultsList.forEach(function (result, i) {
        ...
        elementUnitWidth = utils.roundUpToUnit(svg.info.width, config.padding);
        ...
        spriteData.elements.push({
            className: className,
            id: svg.id = utils.makePath(config.svgId, className, config),
        ...
        x += elementUnitWidth + config.padding;
...
module.exports.templates = {
    ...
    sprite: '<svg baseProfile="tiny" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="{:width:}" height="{:height:}" preserveAspectRatio="xMaxYMax meet" viewBox="0 0 {:width:} {:height:}">{:shapes:}</svg>',
    defs: '<svg{:styles:} xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><defs>{:shapes:}</defs></svg>'
};
